...
const MARKDERS = mockData.schemas
...
export const state: () => State = () => ({
  ...
  settings: [blankSettings(mockData.schemas)],
...
function blankSettings(schemas: Schema[]): Settings {
  return {
    scale: initialScale,
    viewportDateInterval: {
      from: MOSTLEFTDATE,
      to: MOSTLEFTDATE.plus({ days: schemas.length }),
    },
...
export const mutations: MutationTree<State> = {
  ...
  setCascadeIndex(state: State, index: number) {
    ...
    for (let i = 0; i <= index; i++) {
      if (!state.settings[i]) {
        newSettings = true
        state.settings[i] = blankSettings(state.schemas)
      }
    }
    if (newSettings) {
...
export const getters: GetterTree<State, State> = {
  ...
  getRegions(state: State, getters: any): Events {
    ...
    const sortedRegions = sortEvents(
      [...regionsCascade.events],
      getters.settings.sort
    ) as Events
    // const events = getters.events as Events;
  ...
  dateFromClientLeft(state: State, getters: any) {
    return (offset: number) => {
      ...
      return leftDate.plus({
        // days: offset / (getters.settings.viewport.width/ state.schemas.length),
